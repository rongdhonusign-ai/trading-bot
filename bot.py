...
# ==========================================
# 2. CONFIGURATION & COIN EXCLUSIONS
# ==========================================
...
# 🚫 স্টেবলকয়েন, বিটকয়েন এবং অতিরিক্ত ছোট/মেমে টোকেন ফিল্টার
excluded_coins = {
    'USDT', 'USDC', 'BUSD', 'TUSD', 'FDUSD', 'DAI', 'USDE', 
    'BTC', 'BTTC', 'WBTC', 'SATS', '1000SATS', 'SHIB', 'PEPE', 'LUNC'
}

def get_target_altcoins():
    try:
        add_log("🔍 Dynamic market load via CCXT (No IP Spam)...")
        ...
        for symbol, market in markets.items():
            if market['spot'] and market['active'] and market['quote'] == 'USDT':
                ...
                # ফিল্টারিং কন্ডিশন
                if (base not in excluded_coins) and \
                   (not base.startswith('LD')) and \
                   (not base.startswith('1000')): # 1000SATS, 1000BONK জাতীয় টোকেন বাদ
                    
                    ws_sym = symbol.replace('/', '').lower()
                    usdt_pairs.append(ws_sym)
                    
        add_log(f"✅ Found {len(usdt_pairs)} clean Altcoin pairs. Monitoring top 50...")
        return usdt_pairs[:50]
    except Exception as e:
        add_log(f"⚠️ Error loading markets: {e}. Using fallback altcoin list.")
        return [
...
# ==========================================
# 3. TECHNICAL INDICATORS (EMA, RSI, BB)
# ==========================================
...
def process_kline_data(symbol, open_price, close_price, high_price, low_price, is_closed):
    ...
    # ৫ মিনিটের ক্যান্ডেল ক্লোজ হলে ইন্ডিকেটর হিসাব করবে
    if is_closed:
        ...
        if len(df) >= 25:
            ...
            # ১. সবুজ ক্যান্ডেল যাচাই (Close Price > Open Price)
            is_green_candle = c_close > c_open
            
            # ২. ক্যান্ডেল EMA 5 এর নিচে ক্লোজ
            ...
            # 🎯 SELL CONDITION (Upper Bollinger Band-এর উপরে ক্যান্ডেল ক্লোজ)
            ...
            # 💰 SELL EXECUTION (Bollinger Band Cross OR Emergency 2% Stop Loss)
            elif positions[formatted_symbol]:
                # ২% স্টপ লস ক্যালকুলেশন
